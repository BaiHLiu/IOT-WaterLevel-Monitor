    ...
    try:
        ...
        if (not (high_line == 0)) and (not (low_line == 0)):
            ...
            if (water_level < low_line or water_level > high_line):
                ...
                if(not alarm_log):
                    # 没报警过
                    push.send_message(msg_str+f"+ 告警类型：高低水位超限\n+ 水位值：{water_level/1000}米\n+ 预警值:{low_line/1000}/{high_line/1000}")
                    dbconn.set_alarm_log(sensor_id, dev_id, 1, water_level)
                else:
                    # 报警过
                    time_delta = int(((datetime.datetime.now() - datetime.datetime.strptime(alarm_log[5], '%Y-%m-%d %H:%M:%S')).seconds)/60)
                    # 同种类型报警（高低警戒线）
